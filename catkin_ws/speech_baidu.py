...
##########################################################################################
def speech_init(App_ID=None, API_Key=None, Secret_Key=None):
...
speech = speech_init(); opt = {'per':4,'vol':10}
...
def speak(text, speech=speech, dir='TTS', opt=opt):
    ...
    os.makedirs(dir, exist_ok=True) # No Modify
    text = str(text); dst = f'{dir}/{text}.mp3'
    if os.path.isfile(dst): playsound(dst); return
    if type(speech)!=AipSpeech: speech = speech_init()
    ...
    with open(dst,'wb') as fp: fp.write(res) # save audio
...
speak_ = lambda x: Thread(target=speak, args=(x,)).start()
Speak_ = lambda x: Thread(target=Speak, args=(x,)).start()
########################################################
'''def speak_(text, speech=speech, dir='TTS', opt=opt):
    dst = f'{dir}/%s.mp3'%text
    if not os.path.isfile(dst): speak(text, speech, dir, opt)
    else: Thread(target=playsound, args=(dst,)).start()'''
