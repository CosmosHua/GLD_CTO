...
def detect(save_img=False):
    ...
    view_img = True if webcam or source.endswith('.mp4') else view_img

    # Directories
    os.makedirs(save_dir, exist_ok=True)  # make base
    if not webcam:
    ...
    #model(img.half() if half else img) # run once for test
    for path, img, imgs, vid_cap in dataset:
        ...
        # Apply Classifier
        if classify: pred = apply_classifier(pred, modelc, img, imgs)

        # Process detections
        for i, det in enumerate(pred):  # detections per image
            if webcam:  # batch_size >= 1
                p, s, im = Path(path[i]), '%g: '%i, imgs[i].copy()
            else:
                p, s, im = Path(path), '', imgs
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        plot_one_box(xyxy, im, label=label, color=colors[int(cls)], line_thickness=2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-dir', type=str, default='runs', help='directory to save results')
