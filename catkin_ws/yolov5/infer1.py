...
class yolov5_det(object):
    ...
    def infer1(self, im, prepare=True, post=True, show='det'):
        ...
        if not post: return im, pred, (t2-t1)*1000, img

        res = {}; det = pred[0] # batch_size=1
        names = self.names; colors = self.colors
        # Process detections per image
        if det is not None and len(det): # det[0]:(x1,y1,x2,y2,prob,cls)
            # Rescale boxes from img_shape to im_shape(original)
            det[:,:4] = scale_coords(img.shape[2:], det[:,:4], im.shape).round()
            for *xyxy, prob, c in reversed(det): # Add bbox to image
        ...
        if show: cv2.imshow(str(show),im); cv2.waitKey(1)
        return im, res, (t2-t1)*1000
    ...
    def infer(self, src, dst=None, show='det', over=True):
        ...
        for path, img, imgs, vid_cap in data:
            _, pred, dt, img = self.infer1(img, prepare=False, post=False, show=False)
